...
fn parse_error_includes_neb_code() {
    ...
    assert!(
        msg.contains("NEB-S002"),
        "expected parse error code, got: {msg}"
    );
}
...
fn lex_error_includes_neb_code() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("NEB-S001"),
        "expected lex error code, got: {msg}"
    );
}
...
fn deprecated_less_than_is_rejected_with_canonical_hint() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("NEB-S004"),
        "expected deprecated cmp code, got: {msg}"
    );
    assert!(
        msg.contains("lt"),
        "expected canonical operator hint, got: {msg}"
    );
    assert!(
        msg.contains("less than"),
        "expected found synonym, got: {msg}"
    );
}
...
fn deprecated_brace_control_flow_is_rejected() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("NEB-S005"),
        "expected deprecated block code, got: {msg}"
    );
    assert!(
        msg.contains("end"),
        "expected canonical block hint, got: {msg}"
    );
}
...
fn eof_error_includes_neb_code() {
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("NEB-S003"),
        "expected eof error code, got: {msg}"
    );
}
